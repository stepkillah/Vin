...
namespace Test
{
    ...
    public class TestVin
    {
        ...
        {
            var parsedYear = Vin.GetVinYear(vin);
            var manufacturer = Vin.GetWorldManufacturer(vin);
            Assert.AreEqual(year, parsedYear);
            Assert.AreEqual(brand, manufacturer);
        }
        ...
        {
            Assert.AreEqual("International Trucks & Caterpillar Trucks & Chevrolet Silverado 4500HD, 5500HD, 6500HD incomplete vehicle", Vin.GetWorldManufacturer("1HT"));
        }

        [TestMethod]
        public void TestToyotaMatchingFirstTwo()
        {
            Assert.AreEqual("Toyota truck made by TMMI", Vin.GetWorldManufacturer("5TG"));
        }
        ...
        {
            Assert.AreEqual(2019, Vin.GetModelYear("111111111K"), "This should be 1989 because 2019 vehicles do not exist yet (given now is 2016).");
        }
